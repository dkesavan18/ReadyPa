...
import { LocationSelect } from "@/components/customer/LocationSelect";
import { useHotelStore } from "@/store/hotelStore";
import { useLocationStore } from "@/store/locationStore";
import { hotelMatchesLocation, LOCATIONS } from "@/data/locations";
...
export default function CustomerHomePage() {
  ...
  const { hotels, products } = useHotelStore();
  const { selectedLocation } = useLocationStore();

  const locationLabel =
    selectedLocation === "all"
      ? "all areas"
      : LOCATIONS.find((l) => l.id === selectedLocation)?.label ?? "your area";

  const filtered = useMemo(() => {
    return hotels.filter((h) => {
      if (!h.isActive) return false;
      if (!hotelMatchesLocation(h.address, selectedLocation)) return false;
      ...
      if (activeFilter === "veg") {
        return products.some((p) => p.hotelId === h.id && p.isAvailable && p.isVeg);
      }
  ...
  }, [hotels, products, search, activeFilter, selectedLocation]);
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-white sticky top-0 z-30 shadow-soft">
        <div className="flex items-center gap-3 px-4 py-3">
          <LocationSelect />
          <Logo showText={false} iconWidth={28} iconHeight={25} href="/customer/home" />
        </div>
        ...
        {/* Category Filters */}
        <div className="flex gap-2 px-4 pb-4 overflow-x-auto overflow-y-hidden scrollbar-hide">
      ...
      <div className="px-4 py-4 space-y-4 pb-24">
        {isLoading ? (
          ...
          >
            ...
            <p className="text-sm text-gray-500 mt-1">Try another location or filter</p>
          </motion.div>
        ) : (
          <>
            <div className="flex items-center justify-between">
              <p className="text-sm text-gray-500 font-medium">
                ...
                {selectedLocation === "all" ? " nearby" : ` in ${locationLabel}`}
              </p>
            </div>
